...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($application_id <= 0) {
        $error_msg = "Invalid late entry application selected.";
    } elseif (!in_array($status, $allowed_status)) {
        $error_msg = "Invalid status selected.";
    } else {
        $stmt = mysqli_prepare(
            $conn,
            "UPDATE student_late_entry_applications
             SET status = ?,
                 admin_message = ?,
                 reviewed_at = NOW()
             WHERE id = ?"
        ...
        if (mysqli_stmt_execute($stmt)) {
            $success_msg = "Late entry application updated successfully.";
        } else {
            $error_msg = "Failed to update late entry application.";
        }
...
<head>
    <meta charset="UTF-8">
    <title>Late Entry Applications - UniStay</title>
...
<div class="container">
    ...
    <div class="info-box">
        ...
        Admin can approve, reject, or keep late entry applications pending.
    </div>
    ...
    <div class="filter-box">
        <a href="late-entry-applications.php?filter=all" class="filter-link <?php echo $filter === 'all' ? 'active' : ''; ?>">All</a>
        <a href="late-entry-applications.php?filter=pending" class="filter-link <?php echo $filter === 'pending' ? 'active' : ''; ?>">Pending</a>
        <a href="late-entry-applications.php?filter=approved" class="filter-link <?php echo $filter === 'approved' ? 'active' : ''; ?>">Approved</a>
        <a href="late-entry-applications.php?filter=rejected" class="filter-link <?php echo $filter === 'rejected' ? 'active' : ''; ?>">Rejected</a>
    </div>

    <?php if (empty($applications)): ?>
        <div class="empty-box">
            No late entry applications found.
        </div>
    <?php else: ?>
        <?php foreach ($applications as $app): ?>
            <div class="application-card">
                <div class="application-top">
                    <h2><?php echo h($app['student_name']); ?> - Late Entry Request</h2>
                ...
                <div class="details-grid">
                    ...
                    <div class="detail-box">
                        ...
                        <?php echo h($app['entry_date']); ?>
                    </div>

                    <div class="detail-box">
                        <strong>Expected Entry Time</strong>
                        <?php echo h($app['expected_entry_time']); ?>
                    </div>
                ...
                <div class="application-text">
                    ...
                    <strong><?php echo h($app['entry_date']); ?></strong>
                    at <strong><?php echo h($app['expected_entry_time']); ?></strong>.
